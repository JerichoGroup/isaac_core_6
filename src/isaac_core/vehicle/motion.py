...
def _state_to_pose(state: VehicleState) -> GeodeticPose:
    ...
    roll_r, pitch_r, yaw_r = matrix_to_euler(state.rotation)
    return GeodeticPose(
...
def move_forward_backward(
    ...
    start_lat = state.lat_deg
    start_lon = state.lon_deg
    roll_r, pitch_r, _ = matrix_to_euler(state.rotation)
...
def _turn_interpolated(
    ...
    for step in range(1, total_steps + 1):
        ...
        roll_r, pitch_r, yaw_r = matrix_to_euler(mat_interp)
...
def turn_roll(
    ...
    delta_matrix = euler_to_matrix(delta_r, 0.0, 0.0)
...
def turn_pitch(
    ...
    delta_matrix = euler_to_matrix(0.0, delta_r, 0.0)
    target_matrix = compose_rotation(state.rotation, delta_matrix, frame)
    yield from _turn_interpolated(state, target_matrix, duration_s, rate_hz)
...
def turn_yaw(
    ...
    delta_matrix = euler_to_matrix(0.0, 0.0, delta_r)
    target_matrix = compose_rotation(state.rotation, delta_matrix, frame)
    yield from _turn_interpolated(state, target_matrix, duration_s, rate_hz)
...
def turn_to_point(
    ...
    """
    # Compute direction to target in local flat-earth NED
    cos_lat = math.cos(math.radians(state.lat_deg))
    dlat_rad = math.radians(target_lat_deg - state.lat_deg)
    dlon_rad = math.radians(target_lon_deg - state.lon_deg)

    # Compute north and east offsets in metres from flat-earth approximation
    dy = dlat_rad * EARTH_RADIUS_M
    dx = dlon_rad * EARTH_RADIUS_M * cos_lat
    dz = target_alt_m - state.alt_m

    # Yaw: angle from north to the target, measured clockwise (atan2(east, north))
    yaw_r = math.atan2(dx, dy)

    # Pitch: angle up from the horizontal plane
...
def steer(
    ...
    for _step in range(total_steps):
        _roll_r, _pitch_r, current_yaw = matrix_to_euler(current_rotation)
        ...
        if yaw_rate != 0.0:
            ...
            delta_rotation = euler_to_matrix(0.0, 0.0, delta_yaw_r)
            current_rotation = compose_rotation(current_rotation, delta_rotation, RotationFrame.WORLD)

        roll_r, pitch_r, yaw_r = matrix_to_euler(current_rotation)
        yield GeodeticPose(
